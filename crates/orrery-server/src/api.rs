...
pub async fn execute(app: State<App>, Json(req): Json<Request>) -> impl IntoResponse {
    let r = app
        .raft_instance
        .client_write(req)
        .await
...
pub async fn add_learner(app: State<App>, req: Json<(NodeId, String)>) -> impl IntoResponse {
    let node_id = req.0 .0;
    let node = BasicNode {
        addr: req.0 .1.clone(),
    };
...
pub async fn change_membership(app: State<App>, req: Json<BTreeSet<NodeId>>) -> impl IntoResponse {
    let res = app.raft_instance.change_membership(req.0, false).await;
    Json(res)
}

/// Initialize a single-node cluster.
pub async fn init(app: State<App>) -> impl IntoResponse {
...
pub async fn metrics(app: State<App>) -> impl IntoResponse {
...
pub async fn vote(app: State<App>, req: Json<VoteRequest<NodeId>>) -> impl IntoResponse {
